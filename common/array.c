...
byte libaroma_iarray_set_ex(
	...
	item = (LIBAROMA_IARRAY_ITEMP)malloc(sizeof(LIBAROMA_IARRAY_ITEM));
	item->key = key;
	item->val = malloc(sz);
	if (unshift) {
		item->next = a->first;
...
byte libaroma_iarray_free_value(
	LIBAROMA_IARRAYP a) {
	if (!a) {
		return 0;
	}
...
byte libaroma_stack_push(
	...
	LIBAROMA_STACK_ITEMP item = (LIBAROMA_STACK_ITEMP)malloc(sizeof(LIBAROMA_STACK_ITEM));
	item->val = malloc(sz);
	item->next = NULL;
	item->prev = a->last;
	memcpy(item->val, val, sz);
